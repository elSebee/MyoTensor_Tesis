#if defined(MODEL_TYPE_RF)
/*
 * ============================================================
 *  model_rf.cpp — Motor de Inferencia Random Forest
 * ============================================================
 */
...
static Eloquent::ML::Port::RandomForest rf;

bool model_init() {
  Serial.println("[MODEL] Motor Random Forest (micromlgen) inicializado.");
  return true;
}
...
int model_predict(const float *window, int window_size, float &mav, float &rms) {
  ...
  // 2. Inferencia Random Forest directa
  return rf.predict(raw_features);
}
...
void model_log_debug(int raw_pred, int filtered_pred, float mav, float rms) {
  ...
  Serial.printf("[INF-RF] Crudo: %-6s (%d) | Suavizado: %-6s (%d) | MAV: %.4f | RMS: %.4f\n",
